...
REMOTE_PATCH_DIR="/tmp/doctor-preview-av-sync-$$"
...
NEW_IMAGE="doctor-preview-v2:av-sync-2026-04-03"
ROLLBACK_IMAGE="doctor-preview-v2:before-av-sync-2026-04-03"
...
echo "--- sync config"
sudo docker exec "$CONTAINER" env | grep -E '^(WEBRTC_SYNC_MIN_DELAY_MS|WEBRTC_SYNC_MAX_DELAY_MS|WEBRTC_SYNC_SAFETY_MARGIN_MS)='
echo "--- sync hooks"
sudo docker exec "$CONTAINER" grep -n 'class AVSyncController\|play_at\|observe_video_processing_delay\|pc.addTrack(local_audio)' /app/src/webrtc.py
